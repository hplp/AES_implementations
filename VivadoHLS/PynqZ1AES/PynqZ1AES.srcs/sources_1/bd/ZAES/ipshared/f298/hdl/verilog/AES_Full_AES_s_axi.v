...
#(parameter
    ...
    input  wire                          ap_ready,
    input  wire                          ap_idle,
    output wire [0:0]                    cipher_or_i_cipher,
    output wire [7:0]                    Nr,
...
localparam
    ...
    ADDR_IER                       = 6'h08,
    ADDR_ISR                       = 6'h0c,
    ADDR_CIPHER_OR_I_CIPHER_DATA_0 = 6'h10,
    ADDR_CIPHER_OR_I_CIPHER_CTRL   = 6'h14,
    ADDR_NR_DATA_0                 = 6'h18,
    ADDR_NR_CTRL                   = 6'h1c,
    ADDR_DATA_IN_BASE              = 6'h20,
    ADDR_DATA_IN_HIGH              = 6'h2f,
    ADDR_DATA_OUT_BASE             = 6'h30,
    ADDR_DATA_OUT_HIGH             = 6'h3f,
    WRIDLE                         = 2'd0,
    WRDATA                         = 2'd1,
    WRRESP                         = 2'd2,
    WRRESET                        = 2'd3,
...
//------------------------Local signal-------------------
    ...
    reg  [0:0]                    int_cipher_or_i_cipher = 'b0;
    reg  [7:0]                    int_Nr = 'b0;
    // memory signals
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_CIPHER_OR_I_CIPHER_DATA_0: begin
                    rdata <= int_cipher_or_i_cipher[0:0];
                end
...
assign cipher_or_i_cipher = int_cipher_or_i_cipher;
assign Nr                 = int_Nr;
// int_ap_start
always @(posedge ACLK) begin
    if (ARESET)
...
// int_cipher_or_i_cipher[0:0]
always @(posedge ACLK) begin
    if (ARESET)
        int_cipher_or_i_cipher[0:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_CIPHER_OR_I_CIPHER_DATA_0)
            int_cipher_or_i_cipher[0:0] <= (WDATA[31:0] & wmask) | (int_cipher_or_i_cipher[0:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_NR_DATA_0)
            int_Nr[7:0] <= (WDATA[31:0] & wmask) | (int_Nr[7:0] & ~wmask);
    end
end
