...
int main() {
	...
	for (unsigned short test = 0; test < 1; test++) {
		...
		XAxiDma_SimpleTransfer(&axiDMA, (u32) ciphertext_words, AES_WORDS * sizeof(aes_inout), XAXIDMA_DEVICE_TO_DMA);
		//while (XAxiDma_Busy(&axiDMA, XAXIDMA_DEVICE_TO_DMA));
		sleep(4);
		printf("Calculation complete\n");

		printf("ciphertext00=%d\n", ciphertext_words[0].data0);

		//Xil_DCacheInvalidateRange((u32) m_dma_buffer_RX, AES_WORDS * sizeof(aes_inout));
		Xil_DCacheInvalidateRange((u32) ciphertext_words, AES_WORDS * sizeof(aes_inout));
		...
		Xil_DCacheFlushRange((u32) ciphertext_words, AES_WORDS * sizeof(aes_inout));
		//Xil_DCacheFlushRange((u32) m_dma_buffer_RX, AES_WORDS * sizeof(aes_inout));
		Xil_DCacheFlushRange((u32) plaintext_words_d, AES_WORDS * sizeof(aes_inout));
		...
		printf("Receive data from IP core\n");
		//XAxiDma_SimpleTransfer(&axiDMA, (u32) m_dma_buffer_RX, AES_WORDS * sizeof(aes_inout), XAXIDMA_DEVICE_TO_DMA);
		XAxiDma_SimpleTransfer(&axiDMA, (u32) plaintext_words_d, AES_WORDS * sizeof(aes_inout), XAXIDMA_DEVICE_TO_DMA);
		//while (XAxiDma_Busy(&axiDMA, XAXIDMA_DEVICE_TO_DMA));
		sleep(4);
		printf("Calculation complete\n");

		printf("plaintext_d00=%d\n", plaintext_words_d[0].data0);

		//Xil_DCacheInvalidateRange((u32) m_dma_buffer_RX, AES_WORDS * sizeof(aes_inout));
		Xil_DCacheInvalidateRange((u32) plaintext_words_d, AES_WORDS * sizeof(aes_inout));
	}
	if (all_tests_pass)
