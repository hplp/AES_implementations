...
//		cout << endl << endl;
		...
		// encrypt
		AES_Full_axis32(true, (unsigned char) Nr, inputStream, outputStream);
		for (char i = 0; i < stt_lng / 4; i++) {
			ciphertext[4 * i + 3] = (char) (outputStream[i] >> 0);
			ciphertext[4 * i + 2] = (char) (outputStream[i] >> 8);
			ciphertext[4 * i + 1] = (char) (outputStream[i] >> 16);
			ciphertext[4 * i + 0] = (char) (outputStream[i] >> 24);
		}
		cout << "ciphertext = ";
		...
		// decrypt
		AES_Full_axis32(false, (unsigned char) Nr, outputStream, outputStream);
		for (char i = 0; i < stt_lng / 4; i++) {
			decrypted_plaintext[4 * i + 3] = (char) (outputStream[i] >> 0);
			decrypted_plaintext[4 * i + 2] = (char) (outputStream[i] >> 8);
			decrypted_plaintext[4 * i + 1] = (char) (outputStream[i] >> 16);
			decrypted_plaintext[4 * i + 0] = (char) (outputStream[i] >> 24);
		}
