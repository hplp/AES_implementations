...
index = 0
while(message):
	encrypted_message[index:index+16] = AES_Encrypt(message[:16], expandedKey)
	message = message[16:]
	index = index + 16
...
while(encrypted_message):
	decrypted_message[index:index+16] = AES_Decrypt(encrypted_message[:16], expandedKey)
